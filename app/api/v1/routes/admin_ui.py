...
<body>
  ...
  <div class="tabs">
    <button class="tabbtn" data-page="pageAccounts" onclick="showPage('pageAccounts')">Accounts</button>
  ...
  <!-- PAGE: ACCOUNTS -->
  <div id="pageAccounts" class="page card">
    <h3>Accounts</h3>
    ...
    <div class="row">
      ...
      <div>
        ...
        <input id="accKey" placeholder="TEMP_RAW_KEY" style="width:320px" />
      </div>
      <div>
  ...
  <div id="pageQueue" class="page card">
    ...
    <div class="row">
      <div>
        ...
        <input id="queueId" type="number" placeholder="33097944514" style="width:240px" />
      </div>
      <div>
...
async function createAccount() {{
  ...
  const api_key_encrypted = val("accKey").trim();
  const is_active = val("accActive") === "true";

  if (!name || !account_code || !api_key_encrypted) {{
    return alert("Fill name, account_code, api_key_encrypted");
  }}

  setText("accountsStatus", "Creating...");
  await apiFetch("/of-accounts", {{
    method: "POST",
    body: JSON.stringify({{ name, account_code, api_key_encrypted, is_active }})
  }});
  await loadAccounts();
...
function resolveProviderAccountForSend(selectedAccount) {{
  // предпочитаем account_code из DB, иначе fallback input/env
...
async function sendMessage() {{
  ...
  // custom list строго для одного аккаунта (custom_list_id привязан к of_account_id)
  ...
  try {{
    const results = [];
    for (const a of accs) {{
      const providerAccountId = resolveProviderAccountForSend(a);
...
async function loadQueue() {{
  ...
  const providerAccountId = resolveProviderAccountForSend(accs[0]);
